...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['listing_submit'])) {
        ...
        $last_name = safe_input_Text($_SESSION["last_name"]);
        $mobile_number = safe_input_Text($_SESSION["mobile_number"]);
        $email_id = safe_input_Text($_SESSION["email_id"]);
        $register_mode = "Direct";
        // Common Listing Details
        $listing_name = safe_input_Text($_SESSION["listing_name"]);
        $listing_mobile = safe_input_Text($_SESSION["listing_mobile"]);
        $listing_email = safe_input_Text($_SESSION["listing_email"]);
        $listing_website = safe_input_Text($_SESSION["listing_website"]);
        $listing_whatsapp = safe_input_Text($_SESSION["listing_whatsapp"]);
        $listing_address = safe_input_Text($_SESSION["listing_address"]);
        $listing_lat = safe_input_Text($_SESSION["listing_lat"]);
        $listing_lng = safe_input_Text($_SESSION["listing_lng"]);
        ...
        if (isset($_SESSION['listing_type_id']) && !empty($_SESSION['listing_type_id'])) {
            $listing_type_id = safe_input_Text($_SESSION["listing_type_id"]);
        } else {
            $listing_type_id = 1;
        }
        $country_id = safe_input_Text($_SESSION["country_id"]);
        $listing_pincode = safe_input_Text($_SESSION["listing_pincode"]);
        $service_locations = safe_input_Text($_SESSION["service_locations"]);
        ...
        $category_id = safe_input_Text($_SESSION["category_id"]);
        ...
        $opening_days = safe_input_Text($_SESSION["opening_days"]);
        $opening_time = safe_input_Text($_SESSION["opening_time"]);
        $closing_time = safe_input_Text($_SESSION["closing_time"]);
        // Listing Social Link Details
        $fb_link = safe_input_Text($_SESSION["fb_link"]);
        $gplus_link = safe_input_Text($_SESSION["gplus_link"]);
        $twitter_link = safe_input_Text($_SESSION["twitter_link"]);
        // Listing Location Details
        $google_map = safe_input_Text($_SESSION["google_map"]);
        $threesixty_view = safe_input_Text($_SESSION["360_view"]);
        // Listing Video
