...
if (isset($_REQUEST['expert_select_search'])) {
    $select_search = $_REQUEST['expert_select_search']; //Category
} else {
    $select_search = $_GET['select_search']; //Category
}
...
if ($explor_select == 1) {              //Listing Category search
    ...
    $cities_qry = "SELECT * FROM " . TBL . "cities WHERE city_id = '$select_city'";
    ...
    $category_name = $select_search;
    //get matched data from cities table starts
    $cities_qry = "SELECT * FROM " . TBL . "news_cities WHERE city_id = '$select_city'";
    $cities_query = mysqli_query($conn, $cities_qry);
    $cities_row = mysqli_fetch_array($cities_query);
    $city_name = $cities_row['city_name'];
    //get matched data from cities table ends
    ...
    //get matched data from news table
    $news_qry = "SELECT * FROM " . TBL . "news WHERE news_title = '$select_search' AND news_status= 'Active'"; //news Table Fetch
    $news_query = mysqli_query($conn, $news_qry);
    //get matched data from place table
    $places_qry = "SELECT * FROM " . TBL . "places WHERE place_name = '$select_search'"; //places Table Fetch
    $places_query = mysqli_query($conn, $places_qry);
    ...
    if (mysqli_num_rows($categories_query) > 0) {
        ...
        exit;
    } elseif (mysqli_num_rows($categories_query) <= 0 && mysqli_num_rows($listings_query) <= 0 && mysqli_num_rows($event_query) <= 0 && mysqli_num_rows($blog_query) <= 0 && mysqli_num_rows($product_query) <= 0 && mysqli_num_rows($job_query) <= 0 && mysqli_num_rows($news_query) > 0) {
        $news_row = mysqli_fetch_array($news_query);
        $news_code = $news_row['news_id'];
        $news_url = $NEWS_DETAIL_URL . urlModifier($news_row['news_slug']);
        header("location: $news_url");
        exit;
    } elseif (mysqli_num_rows($categories_query) <= 0 && mysqli_num_rows($listings_query) <= 0 && mysqli_num_rows($event_query) <= 0 && mysqli_num_rows($blog_query) <= 0 && mysqli_num_rows($product_query) <= 0 && mysqli_num_rows($job_query) <= 0 && mysqli_num_rows($news_query) <= 0 && mysqli_num_rows($places_query) > 0) {
        $place_row = mysqli_fetch_array($places_query);
        $place_code = $place_row['place_code'];
        $place_url = $PLACE_DETAIL_URL . urlModifier($place_row['place_slug']);
        header("location: $place_url");
        exit;
    } else {
