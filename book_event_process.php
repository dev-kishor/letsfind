...
$checkPass_sql = "SELECT count(*) as count from booked_event_pass where event_id = $event_id and user_id = $userId";
$checkPass_res = mysqli_query($conn, $checkPass_sql);
$checkData = mysqli_fetch_assoc($checkPass_res);
if ($checkData['count'] == 0) {

  $user_sql = "SELECT email_id,first_name FROM " . TBL . "users where user_id = $userId";
  $user_res = mysqli_query($conn, $user_sql);
  $user_data = mysqli_fetch_assoc($user_res);
  $user_email = $user_data["email_id"];
  $user_name = $user_data["first_name"];

  $sql_query = "SELECT u.first_name,u.email_id,event_id,event_name,event_address,event_pincode,event_time,event_start_date FROM " . TBL . "events as e join " . TBL . "users as u on u.user_id = e.user_id where e.event_id = $event_id";
  ...
  $targetDate = $event_data["event_start_date"];
  $event_creator_email = $event_data["email_id"];
  $diffInDays = date("M d", strtotime($targetDate));

  // MAIL TO USER WHO BOOKED EVENT PASS
  $event_name_data = $event_data["event_name"];
  $event_address_data = $event_data["event_address"];
  $event_time_data = $event_data["event_time"];
  $html = '<!DOCTYPE html>
  <html lang="en">
    <head>
      <meta charset="UTF-8" />
      ...
      >
        ...
        >
          <div
            ...
            style="margin: 0; padding: 1.5em; background: #f4f5f6"
          >
            <div
              class="ticket__co"
              style="
            ...
            >
              ...
              >
              <span
                class="u-upper ticket__co-subname"
                style=" color: #506072"
                >Event Pass</span
              >
            </div>
          ...
          <div
            class="ticket__body"
            style="padding: 2rem 1.25em 1.25em; width: 400px"
          >
            <p
              class="ticket__route"
              style="
            ...
            <p
              ...
              "
            >
              <strong>Venue : </strong> ' . $event_address_data . '
            </p>
            <div
              class="ticket__timing"
              style="
                display: flex;
                align-items: center;
                margin-top: 1rem;
            ...
            >
              ...
              >
                <span
                  class="u-upper ticket__small-label"
                  style="
                    display: block;
                    margin-bottom: 0.5em;
                    font-size: 0.875em;
              ...
              <p style="margin: 0; padding: 0; border-right: 0">
                <span
                  class="u-upper ticket__small-label"
                  style="
                    display: block;
                ...
                <span
                  ...
                  style="
                    ...
                    color: #424f5e;
                  "
                  >' . $event_time_data . '</span
                >
              </p>
            </div>
            <p
              class="ticket__fine-print"
              style="
                margin-top: 1rem;
                color: #666;
              "
            >
              This ticket cannot be transferred to another persoon
            </p>
          </div>
        </div>
      </div>
  ...
  $subject = "You are invited | Event Pass";

  $headers1 = "From: " . "$admin_email" . "\r\n";
  $headers1 .= "Reply-To: " . "$admin_email" . "\r\n";
  $headers1 .= "MIME-Version: 1.0\r\n";
  $headers1 .= "Content-Type: text/html; charset=utf-8\r\n";
  mail($user_email, $subject, $html, $headers1);
...
<html lang="en">
  ...
  <body>
    <p>Hello ' . $event_data["first_name"] . ',</p>
    <p>' . $user_name . ' booked a event pass of ' . $event_data["event_name"] . '</p>
  </body>
</html>
';
  $subject2 = "New Event Pass Booked for - " . $event_data["event_name"];
  // MAIL TO USER WHO CREATED EVENT 
  mail($event_creator_email, $subject2, $html2, $headers1);
  $event_id = $event_data["event_id"];
  $insert_into_db = "INSERT INTO `booked_event_pass` (`event_id`, `user_id`) VALUES ('$event_id', '$userId')";
  $insert_res = mysqli_query($conn, $insert_into_db);
  if ($insert_res) {
    echo "1";
  } else {
    echo "0";
