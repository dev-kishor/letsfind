...
if (!empty($_SESSION['city_check'])) {
    if (strstr($_SESSION['city_check'], ',')) {
        ...
        $city_search_row = getCityName($_SESSION['city_check']);
        $city_id = $city_search_row['city_id'];
        $WHERE[] = '(FIND_IN_SET(' . $city_id . ',t1.city_id))';
    }
}
//City Check Ends
...
?>
<h1>bujyb</h1>
<span id="map-error-box" class="map-error-box" style="display:none;"><?php echo $BIZBOOK['LISTINGS_NO_LISTINGS_MESSAGE']; ?></span>
...
<script async defer src="https://maps.googleapis.com/maps/api/js?key=<?php echo $footer_row['admin_geo_map_api']; ?>&callback=initMap">
</script>
<script>
