...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['news_submit'])) {
        ...
        $news_qry = "INSERT INTO " . TBL . "news 
					(news_title, news_description, news_image, news_status,  category_id, city_id,pincode, news_slug, seo_title, seo_description, seo_keywords, news_cdt)
					VALUES 
					('$news_title', '$news_description', '$news_image', '$news_status', '$category_id', '$city_id','$pincode', '$news_slug', '$seo_title', '$seo_description', '$seo_keywords', '$curDate')";
        ...
        if ($news_res) {
            ...
            $message1 =   stripslashes(str_replace(
                array(
                    '\'.$admin_site_name.\'', '\' . $first_name . \'', '\' . $email_id . \'', '\' . $mobile_number . \'', '\' . $news_title . \'', '\' . $news_email . \'', '\' . $news_mobile . \'', '\'.$admin_footer_copyright.\'', '\'.$admin_address.\'', '\'.$webpage_full_link.\'', '\'.$webpage_full_link_with_login.\'', '\'.$admin_primary_email.\''
                ),
                array('' . $admin_site_name . '', '' . $first_name . '', '' . $email_id . '', '' . $mobile_number . '', '' . $news_title . '', '' . $news_email . '', '' . $news_mobile . '', '' . $admin_footer_copyright . '', '' . $admin_address . '', '' . $webpage_full_link . '', '' . $webpage_full_link_with_login . '', '' . $admin_primary_email . ''),
                $mail_template_admin
            ));
