...
function getSlugNewsCategoryLike($arg)
{
    global $conn;
    $sql = "SELECT group_concat(category_id separator ',') as category_id FROM  " . TBL . "news_categories where category_slug like '%" . $arg . "%'";
    $rs = mysqli_query($conn, $sql);
    $row = mysqli_fetch_array($rs);
    return $row;
}
