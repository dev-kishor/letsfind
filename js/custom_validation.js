...
// Send message to user as email
$(".send_msg_to_sender_submit").click(function () {
    $(".send_msg_to_sender_form").validate({
        rules: {
            sender_name: { required: true },
            sender_mobile: { required: true },
        ...
        messages: {
            ...
            sender_mobile: { required: "Mobile Number is Required" },
            sender_email: { required: "Email ID is Required" },
            subject: { required: "Subject is Required" },
            message: { required: "Message is Required" },
        },
        submitHandler: function (form) { // for demo
            if (webpage_full_link != null) {
                var link = webpage_full_link + 'send_mail_msg.php';
            } else {
                var link = 'send_mail_msg.php';
            }
            ...
            $.ajax({
                type: "POST",
                data: $(".send_msg_to_sender_form").serialize(),
                url: link,
                cache: true,
                success: function (html) {
                    if (html == 500) {
                        $("#msg_fail").show();
                        $(".send_msg_to_sender_form")[0].reset();
                    } else {
                        $("#msg_success").show();
                        $(".send_msg_to_sender_form")[0].reset();
